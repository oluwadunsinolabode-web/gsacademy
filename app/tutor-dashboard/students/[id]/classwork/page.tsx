...
export default function StudentClassworkPage() {
  ...
  /*
   * Subjects this tutor is actually assigned
   * to teach this student.
   */
  const [allowedSubjects, setAllowedSubjects] =
    useState<string[]>([]);

  /*
   * Currently selected subject for new classwork.
   */
  ...
  async function loadStudentAndClasswork() {
    try {
      ...
      /*
       * GET SUBJECTS THIS TUTOR IS ASSIGNED
       * TO TEACH THIS STUDENT
       */
      const {
  ...
  async function createClasswork(
    ...
    try {
      ...
      if (createAssignmentError) {
        /*
         * DELETE CLASSWORK IF ASSIGNMENT FAILS
         */
        await supabase
          .from("classworks")
          .delete()
      ...
      {allowedSubjects.length === 0 && (
        <div className="mt-6 rounded-2xl bg-yellow-50 p-5 text-yellow-800">
          ...
          <p className="mt-1 text-sm">
            You can only create classwork
            for subjects you are assigned
            to teach this student.
          </p>
        </div>
      )}
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 font-semibold text-slate-900 outline-none focus:border-yellow-500"
                >
                  {allowedSubjects.map(
                    (subject) => (
                      <option
                ...
                <p className="mt-2 text-xs text-slate-400">
                  Only subjects you are
                  assigned to teach this
